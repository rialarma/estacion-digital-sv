...
const CuentasPorCobrar = () => {
  ...
  const [paymentForm, setPaymentForm] = useState({ amount: '', method: 'EFECTIVO', reference: '' });
  ...
  const fetchDebts = async () => {
    ...
    let query = supabase
      .from('sales')
      .select(`
        id, created_at, total, balance, payment_method,
        clients ( name )
      `)
  ...
  const openPaymentModal = (debt) => {
    ...
    setPaymentForm({ amount: debt.balance.toFixed(2), method: 'EFECTIVO', reference: '' });
    setShowModal(true);
  };
  ...
  const handlePayment = async (e) => {
    ...
    try {
      ...
      // 1. Registrar el pago
      const { error: paymentError } = await supabase.from('cxc_payments').insert([{
      ...
      // 2. Descontar el balance de la venta
      ...
      <div className="glass-panel" style={{ padding: '24px' }}>
        ...
        {loading ? (
          ...
          <div style={{ textAlign: 'center', padding: '60px', color: 'var(--text-muted)' }}>
            ...
            <p>No tienes cuentas pendientes por cobrar a clientes.</p>
          </div>
        ) : (
                  ...
                  <tr key={debt.id}>
                    ...
                    <td>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '8px', fontWeight: 500 }}>
                        ...
                        {debt.clients?.name || 'Cliente Genérico'}
                      </div>
                    </td>
        ...
        <div className="modal-overlay">
          <div className="glass-panel" style={{ width: '100%', maxWidth: '400px', padding: '24px' }}>
            ...
            <div style={{ background: 'rgba(0,0,0,0.2)', padding: '16px', borderRadius: '12px', marginBottom: '20px' }}>
              <p style={{ margin: '0 0 8px 0', fontSize: '13px', color: 'var(--text-muted)' }}>Cliente: <strong>{selectedDebt.clients?.name || 'Genérico'}</strong></p>
            ...
            <form onSubmit={handlePayment} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              <div>
                <label style={{ display: 'block', fontSize: '12px', marginBottom: '6px', color: 'var(--text-muted)' }}>Monto a Abonar ($)</label>
                <input
                  type="number"
                  step="0.01"
                  required
              ...
              <div style={{ display: 'flex', gap: '12px', marginTop: '10px' }}>
                ...
                <button type="submit" className="glass-button" style={{ background: '#10b981', flex: 1 }} disabled={saving}>
